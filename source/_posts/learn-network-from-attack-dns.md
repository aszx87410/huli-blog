...
但要記住的是，這些技術保護的是「你的電腦到 DNS resolver」這段，所以如果你還是用中華電信的 DNS resolver，那中華電信照樣能看到你的 DNS query。

那有沒有方法可以讓 DNS resolver 也不知道呢？

有一個比較新的標準 [RFC 9230: Oblivious DNS over HTTPS](https://www.rfc-editor.org/info/rfc9230/)，簡稱 ODoH，就是為了解決這個問題。

解法很簡單，就是在你的電腦跟 DNS resolver 中間加了一層 proxy。你加密封包以後傳給 proxy，proxy 只看得到你的 IP，但看不到你要查什麼，然後 proxy 把封包轉給 DNS resolver，這時 DNS resolver 雖然看得到查詢內容，但不知道來源 IP。

所以 proxy 知道你的 IP，但不知道你要查什麼；DNS resolver 知道要查什麼，但不知道你是誰，因為所有封包都是從 proxy 來的。只要 proxy 跟 DNS resolver 不是同一個人控制的，就不會得到完整的資訊。（話說在 RFC 裡，其實是分成 proxy 跟 target，這個 target 不一定要是 DNS resolver，是個把封包轉送到 resolver 的另一個 proxy 也行）

Apple 的 [iCloud 私密轉送](https://support.apple.com/zh-tw/102602) 就是這個設計。
...
為了解決這個隱私問題，就出現了 DoT 與 DoH 等機制，透過加密來保護 DNS 查詢的封包，就算攔截到了，也看不到裡面的內容，還有更新的 ODoH，又加了一層保護。
