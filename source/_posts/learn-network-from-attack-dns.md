...
帶著這些問題去閱讀文章，可以更有意識地去理解現在要學的是什麼，待會看到的東西又是為了解決什麼問題。比起平鋪直述的講解名詞，「從一開始就帶著問題閱讀」是我更想嘗試的方式。
...
DNS 全名為 Domain Name System，中文翻叫域名系統，最廣為人知的、針對一般大眾的的解釋，大概就是 DNS 就類似於電話簿，他會把 `google.com` 轉成 `142.250.21.139`，把 `github.com` 轉成 `20.27.177.113`，在網路世界中，最底層拿來發送封包的地址並不是網域，而是 IP，所以必須把網路解析成 IP。
...
由於這個 server 負責把 DNS 解析成 IP，通常稱為 DNS resolver，運作如下：

TODO: 電腦發送請求到 DNS resolver，resolver 回答

雖然對我們來說，就是問他就能得到答案，但其實背後運作不止如此。以我的網域 `blog.huli.tw` 來說，其實是一層一層的。當我們問中華電信時，中華電信會去問掌管所有 DNS 的服務：「root name server」，而這個服務會記錄 `.tw` 是誰管的，跟中華電信說：「去問他吧」。

於是就接下來中華電信就去問管 `.tw` 的 TLD name server，這個 name server 查了 `huli.tw` 之後發現這個網域是由 `ken.ns.cloudflare.com` 管的，就再回說去找他們吧。
...
TODO: DNS 循環解析的過程，以 blog.huli.tw 為例

這個循環解析的過程滿長的，因此不可能每次都這樣，事實是每一個環節都有 cache 的存在，例如說瀏覽器就可能有自己的 cache，作業系統也有，中華電信提供的 DNS resolver 也是，所以並不會每一次都查完整個鏈路才能知道結果，只要碰到 cache 就會返回了。
...
舉例來說，政府發現某個詐騙網站很猖狂，因此直接叫中華電信把它們的 domain 解析成政府單位的 IP，因此使用者連到該網誌時，會顯示警告畫面，看不到原本的內容。
...
TODO: 透過中華電信解析以及透過 8.8.8.8 解析的差異
...
像這樣主動干預網路世界的行為，想當然爾也有許多法規以及制度上的討論，到底政府能介入到什麼程度？又該怎麼防止 RPZ 胡亂封鎖？也有人認為政府壓根就不該干預，網路世界應該是自由的。
...
但這並不是強制的，所以找到沒有做 BCP 38 的 ISP 以後，你就能偽造來源 IP，發送請求給 name resolver。
...
TODO: demo 網頁，示範猜測以及要等 cache 過期
...
這有一點類似剛提過的買一送一，但有本質上的區別。買一送一是：「你問我 `huli.tw`，我順便告訴你 `gooogle.com`」，但這根本不該發生，所以被封掉了。
...
為什麼可以預測呢？因為大部分的隨機數都不是真的隨機數，而是「假裝成隨機數」，例如說有個簡單的產生隨機數的演算法叫 LGC（Linear Congruential Generator），寫成 JavaScript 程式碼是：
...
這其實就是解聯立方程式啦，跟 `2x+y+z=3`、`5x+2y+3z=2`、`x+y+z=9`，請問 xyz 是多少意思。
...
隨機數被破解以後，port 跟 tranasction ID 都一猜就中，40 億次的可能性被你偷看未來以後縮減到只剩下 1，前面加的保護措施都沒用了。而解法的話，就是把隨機數換成更安全的產生方式，以及重要的跟不重要的隨機數不要混在一起。

像這個例子就是重要的隨機數如 transaction ID 跟 port，用的 PRNG instance 跟那些不重要的隨機數如 RRSet 排序相同，所以你可以從其他地方反推回去 seed 是什麼。

這個漏洞已經在去年 10 修復，官方公告在這：[CVE-2025-40780: Cache poisoning due to weak PRNG](https://kb.isc.org/docs/cve-2025-40780)。
...
TODO: DNS cookie 的運作 demo

當這個關係建立後，DNS resolver 如果收到偽造的請求，由於攻擊者不知道 client cookie，也沒辦法偽造 server cookie，所以驗證就不會通過。

那在上一個小結討論的猜測隨機數的手法，是怎麼繞過這個 DNS cookie 的防禦呢？

雖然 DNS cookie 這個機制在 BIND 裡面預設就是開啟的，但是新版的過程時間是 60 秒，每 60 秒就會把整個流程重來一次，因此攻擊者每 60 秒就有一次機會攻擊，只要你能比權威 DNS 更快回應，就能先傳送你自己的 client cookie，進而繞過這個保護機制。

所以 DNS cookie 這整個機制看似沒問題，但因為過期時間太短了，所以在剛過程後的那一個 window 裡面，就等於是沒這個防護的。
...
前面試過的方法其實都治標不治本，一開始 transaction ID 能被猜中，所以加了個隨機的 port，就猜不到了，然後也有了 DNS cookie，再多一個你猜不到的 client cookie 外加 server cookie，以第一個連線的人為準。
...
也因為 DNS 封包沒加密，因此你所使用的 DNS resolver 或甚至這整條鏈路上的節點，都能看到你的封包，知道你查詢了哪些網站，洩漏了你的隱私。

因此呢，就有兩個技術出現了，一個是 DNS over TLS，簡稱 DoT，把 TLS 那套搬到 DNS 查詢身上，這次改走 TCP 連線外加 TLS 驗證，跟 HTTPS 類似，讓其他人都看不到封包內容。

另一個技術是 DNS over HTTPS，簡稱 DoH，把 DNS 查詢那套換成 HTTPS，例如說要查 DNS 時就用 `POST /dns-query` 發送請求，也能達到防止隱私外洩的後果。像是 Google 的[公用 DNS](https://developers.google.com/speed/public-dns/docs/doh?hl=zh-tw) 就有提供這個功能：
